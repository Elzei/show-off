...
// Define types of coins. Reference value: 1.0 = gold. Each coin weighs 0.031 units.
MoneyTypes[] =
    { {"platinum",10, 0.031},    {"gold",     1, 0.031},    {"silver", 0.6,  0.031},
      {"bronze", 0.4, 0.031},    {"copper", 0.2, 0.031},    {"wood",   0.01, 0.031} },
...
//Game generate room when player come in to new room.
//Game will be remember previous room, but will no generate whole maze
//when game is started. Simply:
//player come in to room, game generate room and room next to current room.
struct Maze
{
...
{
    if(!maze.GenerateRoom(wherex, wherey, model, 0).Wall) 
		return true;
    return false;
}
...
static Room& SpawnRooms(long wherex,long wherey, const Room& model = defaultroom)
{
    ...
    for(int o=1; o<5 && CanMoveTo(wherex,wherey+o, room); ++o) 
		Spawn4rooms(wherex,wherey+o);
    for(int o=1; o<5 && CanMoveTo(wherex,wherey-o, room); ++o) 
		Spawn4rooms(wherex,wherey-o);
    for(int o=1; o<6 && CanMoveTo(wherex-o,wherey, room); ++o) 
		Spawn4rooms(wherex-o,wherey);
    for(int o=1; o<6 && CanMoveTo(wherex+o,wherey, room); ++o) 
		Spawn4rooms(wherex+o,wherey);
    return room;
}
...
help:
    ...
    while(life > 0)
    {
        ...
        else if(rm(s, "((go|walk|move) +)?(n|north)"_r)) 
		{ 
			if(TryMoveBy( 0,-1)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(s|south)"_r)) 
		{ 
			if(TryMoveBy( 0, 1)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(w|west)"_r))  
		{ 
			if(TryMoveBy(-1, 0)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(e|east)"_r))  
		{ 
			if(TryMoveBy( 1, 0)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(nw|northwest)"_r)) 
		{ 
			if(TryMoveBy(-1,-1)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(ne|northeast)"_r)) 
		{ 
			if(TryMoveBy( 1,-1)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(sw|southwest)"_r)) 
		{ 
			if(TryMoveBy(-1, 1)) 
				Look(); 
		}
        else if(rm(s, "((go|walk|move) +)?(se|southeast)"_r)) 
		{ 
			if(TryMoveBy( 1, 1)) 
				Look(); 
		}
    ...
    std::cout
        << "%s\n"_f % (life<0
            ? "You are pulled out from the maze by a deus ex machina!"
            : "Bye bye.")
        << "[life:%ld] Game over\n"_f % life;
}