...
class Application(Frame):
        ...
        def create_widgets(self):
                """     
                Utwórz widżet potrzebny do pobrania informacji podanych
                ...
                #utwórz etykietę i pole znakowe do wpisania imienia osoby
                Label(self, text = "Osoba: ").grid(row = 1, column = 0, columnspan = 2, sticky = W)
                self.person_ent = Entry(self)
                self.person_ent.grid(row = 1, column = 1, sticky = W)

                #utwórz etykietę i pole znakowe do wpisywania rzeczownika w liczbie mnogiej
                Label(self, text = "Rzeczownik w liczbie mnogiej: ").grid(row = 2, column = 0, columnspan = 2, sticky = W)
                self.noun_ent = Entry(self)
                self.noun_ent.grid(row = 2, column = 1, sticky = W)
                
                #utwórz etykietę i pole znakowe służące do wpisywania czasownika
                Label(self, text = "Czasownik: ").grid(row = 3, column = 0, sticky = W)
                self.verb_ent = Entry(self)
                self.verb_ent.grid(row = 3, column = 1, sticky = W)
                
                #utwórz etykietę do pól wyboru przymiotników
                Label(self, text = "Przymiotnik(i): ").grid(row = 4, column = 0, sticky = W)

                #utwórz pole wyboru przymiotnika "naglące"
                self.is_itchy = BooleanVar()
                Checkbutton(self, text = "naglące", variable = self.is_itchy).grid(row = 4, column = 1, sticky = W)

                #utwórz pole wyboru przymiotnika "radosne"
                self.is_joyous = BooleanVar()
                Checkbutton(self, text = "radosne", variable = self.is_joyous).grid(row = 4, column = 2, sticky = W)

                #utwórz pole wyboru przymiotnika "elektryzujące"
                self.is_electric = BooleanVar()
                Checkbutton(self, text = "elektryzujące", variable = self.is_electric).grid(row = 4, column = 3, sticky = W)

                #utwórz etykietę do przycisków opcji odnoszących się do części ciała
                Label(self, text = "Część ciała:").grid(row = 5, column = 0, sticky = W)

                #utwórz zmienną na pojdyńczą część ciała
                self.body_part = StringVar()
                self.body_part.set(None)

                #utórz przycisk opcji odnoszące się do części ciała
                ...
                Button(self, text = "Kliknij aby wyświetlić opowiadanie", comman = self.tell_story).grid(row = 6, column = 0, sticky = W)
        ...
        def tell_story(self):
                """
                Wpisz w pole tekstowe nowe opowiadanie oparte na danych użytkownka.
                """
                #pobierz wartość interfejsu GUI
